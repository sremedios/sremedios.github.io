import * as wasm from './qa_bg.wasm';
...
export function putImageData(canvas, ctx, new_image) {
    try {
        _assertClass(new_image, Lightbox);
        var ptr0 = new_image.ptr;
        new_image.ptr = 0;
        wasm.putImageData(addBorrowedObject(canvas), addBorrowedObject(ctx), ptr0);
    } finally {
        heap[stack_pointer++] = undefined;
        heap[stack_pointer++] = undefined;
    }
...
export function open_image(canvas, ctx) {
    ...
    return Lightbox.__wrap(ret);
}

/**
* @param {Lightbox} lb
*/
export function rev(lb) {
    _assertClass(lb, Lightbox);
    wasm.rev(lb.ptr);
}
...
export class Lightbox {
    ...
    static __wrap(ptr) {
        const obj = Object.create(Lightbox.prototype);
        obj.ptr = ptr;
    ...
    * @returns {Lightbox}
    */
    static new(raw_pixels, width, height) {
        var ptr0 = passArray8ToWasm0(raw_pixels, wasm.__wbindgen_malloc);
        var len0 = WASM_VECTOR_LEN;
        var ret = wasm.lightbox_new(ptr0, len0, width, height);
        return Lightbox.__wrap(ret);
    }
}
